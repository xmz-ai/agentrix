...
## Repository-Local Filesystem Agents

For CI or repository-specific automation, you can keep an agent inside the target repository instead of registering it in Agentrix:
...
When a run enables filesystem agent resolution, Agentrix resolves the agent in this order:

1. `<workspace>/.agentrix/agents/<agentId>/`
2. `~/.agentrix/agents/<agentId>/`

Use `agentrix-run` with `--allow-filesystem-agent` to run a repository-local agent:
...
npx --yes @agentrix/agentrix-run@0.2.0 \
  ...
  --prompt "Triage this repository." \
  --response-mode stream \
  --runner-id machine-xxxx
```

Filesystem agents are loaded from disk by the runner. They do not need a database Agent/DraftAgent record and do not use the published-agent git install flow.
...
See [Hooks Development Guide](hooks/3.development-guide.md) for more details.
...
1. **Learn Agent Structure**: Read [Agent Structure](3.agent-structure.md)
2. **Configure Agent**: See [Configuration Guide](4.configuration.md)
3. **Add Hooks**: Explore [Hooks System](hooks/1.overview.md)
4. **Follow Best Practices**: Review [Best Practices](5.best-practices.md)
...
See [complete examples](hooks/8.examples.md) for real-world agent patterns.
