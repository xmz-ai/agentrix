...
An Agentrix agent is a git repository with a specific structure that supports multiple AI frameworks (Claude, Codex, Pi) while maintaining consistency.
...
interface AgentMetadata {
  ...
  frameworks?: ('claude' | 'codex' | 'pi')[]; // Supported frameworks
}
```
...
Pi agents use Pi's native resource layout. `AGENTS.md`, `skills/`, `prompts/`, and `extensions/`
inside `.pi/` are discovered by the Pi SDK. Provider credentials and user-wide model settings stay
in `~/.pi/agent/`; do not commit them into an agent repository.
