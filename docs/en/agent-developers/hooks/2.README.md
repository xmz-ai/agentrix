...
Agentrix supports the Claude SDK hook types:
...
Claude hook input types are provided by `@anthropic-ai/claude-agent-sdk`:
...
5. **Type Safety**: Import hook input types from `@anthropic-ai/claude-agent-sdk`; use `@agentrix/shared` for `HookFactory` and `AgentrixContext`
