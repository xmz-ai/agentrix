...
### 3. **Prompt and Session Policy**

```typescript
// Add a lightweight audit note when a prompt is submitted
export async function UserPromptSubmit(input: UserPromptSubmitHookInput) {
  console.log(`[UserPromptSubmit] ${input.prompt.slice(0, 80)}`);
  return {};
}
```
...
Claude hook input types come from the Claude Agent SDK:
...
Agentrix context helper types come from `@agentrix/shared`:
