...
- Basic understanding of [hooks concepts](1.overview.md)
...
- `@agentrix/shared`: Provides `HookFactory` and `AgentrixContext` when hooks need task/workspace context
...
See [Hook Types Reference](4.hook-types.md) for complete type definitions.
...
1. **Type Safety**: Import types from correct packages (`@anthropic-ai/claude-agent-sdk` for Claude SDK hook inputs, `@agentrix/shared` for `HookFactory` and `AgentrixContext`)
...
- [Hooks Overview](1.overview.md) - Conceptual overview
- [Agent Structure](../3.agent-structure.md) - Hooks directory structure
