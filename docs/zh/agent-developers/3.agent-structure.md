...
Agentrix 代理是一个具有特定结构的 git 仓库，它支持多种 AI 框架（Claude、Codex、Pi），同时保持一致性。
...
Pi Agent 使用 Pi 原生资源布局。Pi SDK 会发现 `.pi/` 中的 `AGENTS.md`、`skills/`、
`prompts/` 和 `extensions/`。提供商凭据和用户级模型设置仍保存在 `~/.pi/agent/`，不要将其
提交到 Agent 仓库。
...
详情请参阅[钩子文档](hooks/1.overview.md) 。
