...
*   `@agentrix/shared`: 当钩子需要任务/工作区上下文时，提供 `HookFactory` 和 `AgentrixContext`
...
1.  **类型安全** : 从正确的包中导入类型（`@anthropic-ai/claude-agent-sdk` 用于 Claude SDK 钩子输入，`@agentrix/shared` 用于 `HookFactory` 和 `AgentrixContext`）
