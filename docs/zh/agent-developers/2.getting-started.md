...
## 仓库内 filesystem agent

对于 CI 或仓库专属自动化，可以把 agent 放在目标仓库内，而不是先注册到 Agentrix：
...
当 run 开启 filesystem agent 解析时，Agentrix 会按以下顺序查找 agent：

1. `<workspace>/.agentrix/agents/<agentId>/`
2. `~/.agentrix/agents/<agentId>/`

通过 `agentrix-run` 运行仓库内 agent 时，需要传 `--allow-filesystem-agent`：
...
Filesystem agents 由 runner 从磁盘加载，不需要数据库里的 Agent/DraftAgent 记录，也不走已发布 agent 的 git 安装流程。
